...
% Set dc term pos (middle pixel) and 5x5 area around it to 0
[P,Q] = size(F2);
x = floor(P/2)+1;
y = floor(Q/2)+1;

F2((x-2):(x+2),(y-2):(y+2)) = 0;

maxVal = max(F2(:));
[rs,cols] = find(F2 == maxVal);

r=rs(1,1); % the row number of one the two dominant peaks (use the first occurence)
c=cols(1,1); % the column number of the same peak as above (=||=)
...
% Center coordinates stored in x & y from above
uk= r - x;  % uk and vk are the positions of the peaks relative the center of the spectrum
vk= c - y; %
